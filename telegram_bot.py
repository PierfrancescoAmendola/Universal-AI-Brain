...
def parse_and_ingest_json_payload(raw_text: str, user_name: str) -> Optional[str]:
    ...
    with get_db() as conn:
        cross_links_to_add = []
        for n in nodes_list:
            slug = str(n.get("id", "")).strip().lower().replace(" ", "-")
            if not slug:
                continue
            label = str(n.get("label", slug)).strip()
            ...
            if isinstance(tags, list):
                tags_str = json.dumps([str(t).strip().lower() for t in tags if str(t).strip()])
            else:
                tags_str = json.dumps(["json-import"])

            summary = str(n.get("summary", label)).strip()
